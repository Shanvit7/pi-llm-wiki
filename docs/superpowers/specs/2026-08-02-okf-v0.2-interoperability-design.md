...
│   ├── index.md                # okf_version: "0.2", profile metadata
│   ├── log.md
│   ├── sources/
│   ├── concepts/
│   ├── entities/
...
The root index declares the base format and producer profile:
...
Plain OKF bundles without the profile are fully supported. The profile documents pi-llm-wiki extension fields and page-type conventions; it does not make them mandatory for imported documents.
...
- add or update root OKF version/profile metadata
